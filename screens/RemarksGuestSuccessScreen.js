...
const RemarksGuestSuccessScreen = props => {
  const dimensions = useWindowDimensions();
  const Constants = GlobalVariables.useValues();
  const Variables = Constants;

  const getticketstatuGuestFun = regNo => {
    ...
    >
      ...
      >
        ...
        <View>
          ...
          >
            {props.route?.params?.remarksGuestMsg ?? ''}
          </Text>
          {/* Empty Remarks Message */}
          <>
            {!(
              (props.route?.params?.remarksGuestMsg ?? '')?.length === 0
            ) ? null : (
              <Text
                style={StyleSheet.applyWidth(
                  {
...
export default withTheme(RemarksGuestSuccessScreen);
